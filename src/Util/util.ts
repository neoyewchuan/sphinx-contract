export default class SphinxUtils    {

    static calculateWarrantyEndDate(warStart: string, warPeriod = 12): string {
        ...
        return new String(theYear).trim()
            .concat("-", ("00" + (theMonth).toString().trim()).slice(-2), "-",
    ...
    static dayInMonth(theYear: number, theMonth: number): number    {
        if (theMonth == 2)  {
            if (theYear % 4 === 0 && theYear % 100 !== 0)   {
                // is a leap year
                return 29;
